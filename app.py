...
<style>
:root { --primary-purple: #7F56D9; --light-purple: #F9F5FF; --dark-purple: #42307D; --text-dark: #1D2939; --premium-pink: #FF4757; --success-green: #2ECC71; }
...
def chat():
 ...
 mode = data.get("mode", "AI Tutor")
 simulated_answers = {"bacteria": "Bacteria are microscopic, single-celled organisms that thrive in diverse environments. They can live in soil, the ocean, and inside the human body. Some bacteria are harmful, while others are highly beneficial for digestion and making curd!", "web design": "To create a great website structure, you should master HTML for content, CSS for purple-themed styles, and JavaScript for user interactions. Keep your layout clean and mobile-friendly.", "video editor": "Excellent video editing requires cutting unnecessary pauses, adjusting audio levels for clarity, and adding smooth transitions between clips."}
 q_lower = user_question.lower()
 found_key = "bacteria" if "bacteria" in q_lower else ("web" if "web" in q_lower else ("video" if "video" in q_lower else None))
 if found_key and found_key in simulated_answers: answer = simulated_answers[found_key]
 else: answer = f"Here is the smart breakdown for your request regarding '{user_question}': This topic falls under the {mode} category. Please research standard reference books for extended charts, or ask specific sub-questions like definitions and formulas!"
